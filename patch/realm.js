...
const realmPatchDir = '../patch/realm/';
const realmModuleDir = '../node_modules/realm/';

if (!fs.pathExistsSync(realmPatchDir)) fs.mkdirsSync(realmPatchDir);
...
function getTempDir() {
  return realmPatchDir;
  // return process.env.REALM_DOWNLOAD_CORE_TEMP_DIR || path.join(os.tmpdir(), TEMP_DIR_SUFFIX);
}
...
function download(serverFolder, archive, destination) {
  // Giangdv check exist patch file
  const patchRealm = `${realmPatchDir}${archive}`;
  if (fs.pathExistsSync(patchRealm)) return patchRealm;

  // Download
  console.log('File is not exist: ', patchRealm);
  const url = `https://static.realm.io/downloads/${serverFolder}/${archive}`;
  console.log(`Download url: ${url} => ${destination}`);
...
function extract(downloadedArchive, targetFolder, archiveRootFolder) {
  ...
  if (!archiveRootFolder) {
    ...
    return tar
      .extract({ file: downloadedArchive, cwd: tempExtractLocation })
...
function acquire(desired, target) {
  console.log('Patch realm for debug');
...
const vendorDir = path.resolve(__dirname, `${realmModuleDir}vendor`);
...
const dependencies = ini.parse(fs.readFileSync(path.resolve(__dirname, `${realmModuleDir}dependencies.list`), 'utf8'));
